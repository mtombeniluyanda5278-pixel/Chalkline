...
async function verifyEmail(
  ...
  const dom = new JSDOM(
    ...
    {
      url: "http://127.0.0.1:3000/#/verify-email",
  ...
  globalThis.fetch = async (path, options = {}) => {
    ...
    return Response.json({ passkeys: [], sessions: [], preferences: {} });
  };
  ...
  await until(() =>
    verified
      ? location.hash === "#/account" || document.querySelector(".form-success")
      : !document.querySelector(".form-error").hidden,
  );
...
test("email verification refreshes guest auth and navigation from the issued session", async (t) => {
  ...
  assert.deepEqual(
    requests.slice(0, 3).map(({ path }) => path),
...
test("invalid verification codes remain errors and do not refresh auth", async (t) => {
  const { app, requests } = await verifyEmail(t, {
    response: () =>
      Response.json(
