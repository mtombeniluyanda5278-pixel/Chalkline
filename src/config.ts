...
import { z } from "zod";
const bytes = (value: number) => z.coerce.number().int().positive().default(value);
const bool = (value: boolean) => z.enum(["true", "false"]).default(String(value) as "true" | "false").transform(v => v === "true");
const Env = z.object({
  NODE_ENV: z
    .enum(["development", "test", "production"])
    .default("development"),
  APP_NAME: z.string().default("Chix"),
  OUTBOX_ENCRYPTION_KEY: z.string().regex(/^[a-fA-F0-9]{64}$/).optional(),
  RATE_LIMIT_KEY_SECRET: z.string().min(64).optional(),
  SESSION_DEFAULT_DAYS: z.coerce.number().refine(v => [1,7,30,90].includes(v)).default(30),
  SESSION_IDLE_MAX_DAYS: z.coerce.number().int().min(1).max(14).default(14),
  BOT_PROTECTION_PROVIDER: z.enum(["turnstile","hcaptcha","mock","disabled"]).default("disabled"),
  ...
  UNVERIFIED_FILE_STORAGE_QUOTA_BYTES: z.coerce.number().int().nonnegative().default(0),
  ...
  MALWARE_SCANNER_MODE: z.enum(["clamav","mock","disabled"]).default("disabled"),
...
if (config.MALWARE_SCANNER_MODE === "clamav" && !config.CLAMAV_HOST) throw new Error("CLAMAV_HOST is required.");
if (isProd) {
  if (!config.OUTBOX_ENCRYPTION_KEY || !config.RATE_LIMIT_KEY_SECRET) throw new Error("Dedicated outbox and rate limit keys are required.");
  if (config.MALWARE_SCANNER_MODE === "mock" || config.BOT_PROTECTION_PROVIDER === "mock") throw new Error("Mock providers are forbidden in production.");
  if (config.BOT_REQUIRE_REGISTRATION && (config.BOT_PROTECTION_PROVIDER === "disabled" || !config.BOT_PROTECTION_SECRET_KEY || !config.BOT_PROTECTION_SITE_KEY)) throw new Error("Registration requires a configured CAPTCHA provider.");
