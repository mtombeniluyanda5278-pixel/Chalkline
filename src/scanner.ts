...
export function createClamAvScanner(options: {
  ...
  async function request(
    command: string,
    timeout: number,
    send?: (socket: Socket, signal: AbortSignal) => Promise<void>,
    signal?: AbortSignal,
  ) {
    ...
    return new Promise<string | null>((resolve) => {
      ...
      const abort = () => finish(null),
        deadline = setTimeout(abort, timeout);
      ...
      socket.on("connect", () => {
        void (async () => {
          ...
          if (send) {
            await send(socket, cancel.signal);
            sent = true;
          }
        })().catch(abort);
      });
  ...
  return {
    privacy: "PRIVATE",
    async health() {
      return (await request("zPING\0", options.healthTimeoutMs)) === "PONG";
    },
    async scan(path, signal) {
          ...
          try {
            for await (const chunk of input) {
              ...
              if (!socket.write(Buffer.concat([header, chunk])))
                await once(socket, "drain", { signal: cancel });
            }
      ...
      if (result && /^stream: .+ FOUND$/.test(result))
        return {
          ...base,
          status: "INFECTED",
          detection: result
...
export function scanner(): PrivateScanner {
  if (config.MALWARE_SCANNER_MODE === "clamav")
    return createClamAvScanner({
      host: config.CLAMAV_HOST!,
      port: config.CLAMAV_PORT,
      timeoutMs: config.SCANNER_TIMEOUT_MS,
      healthTimeoutMs: config.SCANNER_HEALTH_TIMEOUT_MS,
      maxBytes: config.UPLOAD_MAX_BYTES,
    });
  return {
    privacy: "PRIVATE",
    async health() {
      return config.MALWARE_SCANNER_MODE === "mock";
    },
    async scan(path, signal) {
      if (config.MALWARE_SCANNER_MODE !== "mock")
        return { status: "ERROR", engine: "disabled", privacy: "PRIVATE" };
      let carry = "",
        infected = false,
        error = false,
        bytes = 0;
      try {
        for await (const b of createReadStream(path, { signal })) {
          bytes += b.length;
          if (bytes > config.UPLOAD_MAX_BYTES) throw new Error("Size limit");
          carry = (carry + b.toString()).slice(-100000);
          infected ||= carry.includes("EICAR-STANDARD-ANTIVIRUS-TEST-FILE");
          error ||= carry.includes("CHIX_TEST_SCANNER_ERROR");
        }
      } catch {
        error = true;
      }
      return {
        status: error ? "ERROR" : infected ? "INFECTED" : "CLEAN",
        engine: "mock-test-only",
        privacy: "PRIVATE",
        ...(infected ? { detection: "test-signature" } : {}),
      };
    },
  };
}
