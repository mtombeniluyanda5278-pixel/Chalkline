...
for i=1,#KEYS do redis.call('ZADD',KEYS[i],now+tonumber(ARGV[1]),ARGV[2]);redis.call('PEXPIRE',KEYS[i],ARGV[1]) end
return 1`;
export async function concurrencyLease(
  limits: { key: string; max: number }[],
  timing = { ttlMs: 120000, renewMs: 30000 },
): Promise<ConcurrencyLease | null> {
  if (redis.status !== "ready")
    throw failure(503, "Transfer capacity is temporarily unavailable.");
  if (!limits.length || timing.renewMs >= timing.ttlMs / 2)
    throw new Error("Invalid lease configuration");
  const token = randomUUID(),
    keys = limits.map((limit) => limit.key),
  ...
  const lost = () => {
    ...
    clearTimeout(expiry);
    controller.abort(new Error("Concurrency lease lost"));
  };
  // If Redis stalls, stop the transfer before the last confirmed lease can expire.
  let expiry = setTimeout(
    lost,
    Math.max(1, timing.ttlMs - (Date.now() - started) - timing.renewMs),
  );
  const interval = setInterval(() => {
    ...
    void redis
      .eval(renew, keys.length, ...keys, timing.ttlMs, token)
      .then((result) => {
        ...
        expiry = setTimeout(
          lost,
          Math.max(1, timing.ttlMs - (Date.now() - began) - timing.renewMs),
        );
        expiry.unref();
      })
      .catch(lost)
...
export async function transferLease(
  ...
  const max =
    kind === "upload"
      ? [config.MAX_UPLOADS_GLOBAL, config.MAX_UPLOADS_PER_USER]
      : [config.MAX_DOWNLOADS_GLOBAL, config.MAX_DOWNLOADS_PER_USER];
  const lease = await concurrencyLease([
    { key: `active:${kind}:global`, max: max[0]! },
    { key: `active:${kind}:${opaqueIdentity(userId)}`, max: max[1]! },
  ]);
  if (!lease)
    throw Object.assign(
      failure(429, "Too many active transfers. Retry shortly."),
      { retryAfter: 5 },
    );
  return lease;
}
export function scanLease() {
  return concurrencyLease([
    { key: "active:scan:global", max: config.MAX_SCANS_GLOBAL },
  ]);
}
