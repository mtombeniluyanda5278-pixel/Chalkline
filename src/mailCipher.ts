...
function keyBytes(key: string): Buffer {
  if (!/^[a-fA-F0-9]{64}$/.test(key))
    throw new Error("Invalid outbox encryption key configuration.");
...
export function openMail(
  ...
  if (!/^[A-Za-z0-9+/]+={0,2}$/.test(encoded))
    throw new Error("Invalid encrypted mail.");
  ...
  for (const candidate of [key, previousKey]) {
    ...
    try {
      ...
      /* Never include the envelope or a key in errors. */
    }
  }
  throw new Error(
    "Encrypted mail could not be opened with the configured keys.",
  );
}
