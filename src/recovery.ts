...
import { recoveryProtection } from "./bot.js";
import { lockSessionUser } from "./sessions.js";
export const maskedEmail = (email: string | null) =>
  email
    ? `${email.slice(0, 1)}***@${email.split("@")[1]?.slice(0, 1)}***.${email.split(".").at(-1)}`
    : null;
...
export async function registerRecoveryRoutes(app: FastifyInstance) {
  app.get("/v1/me/recovery-email", async (req, reply) => {
    ...
    const row = (
      await pool.query(
        "SELECT recovery_email,pending_recovery_email FROM users WHERE id=$1",
        [user.id],
      )
    ).rows[0];
    return {
      email: maskedEmail(row.recovery_email),
      pendingEmail: maskedEmail(row.pending_recovery_email),
    };
  });
  app.post("/v1/me/recovery-email", async (req, reply) => {
    ...
    await transaction(async (c) => {
      ...
      if (user.email.toLowerCase() === b.email)
        throw failure(
          400,
          "Choose an address different from your primary email.",
        );
      await c.query("UPDATE users SET pending_recovery_email=$2 WHERE id=$1", [
        session.id,
        b.email,
      ]);
  ...
  app.post("/v1/auth/verify-recovery", async (req) => {
    await limit("recoveryIp", req.ip);
    const b = parse(Token, req.body);
    await transaction(async (c) => {
      ...
      if (
        user.pending_recovery_email !== token.target_email ||
        user.email === token.target_email
      )
        throw failure(400, "Invalid or expired link.");
      if (user.recovery_email && user.recovery_email !== token.target_email)
        await deliverDevOrLogEmail(
          {
            userId: user.id,
            to: user.recovery_email,
            subject: "Chix recovery email changed",
            body: "Your recovery email was changed. Review account security if you did not request this.",
          },
          c,
        );
      // Recovery proof is bound to the address that was current when issued.
      // Retiring these tokens also prevents change-away/change-back replay.
      await c.query(
        "UPDATE email_tokens SET used_at=now() WHERE user_id=$1 AND channel='recovery' AND used_at IS NULL",
        [user.id],
      );
      await c.query(
        ...
        await deliverDevOrLogEmail(
          {
            ...
            subject: "Chix recovery email removed",
            body: "This recovery email was removed from your account.",
          },
          c,
        );
      await c.query(
        "UPDATE users SET recovery_email=NULL,recovery_verified_at=NULL,pending_recovery_email=NULL WHERE id=$1",
        [session.id],
      );
      await c.query(
  ...
  app.post("/v1/auth/change-email", async (req) => {
    ...
    await transaction(async (c) => {
      ...
      if (user.recovery_email === token.target_email)
        throw failure(400, "Primary and recovery addresses must differ.");
      if (
        (
          await c.query("SELECT 1 FROM users WHERE email=$1", [
            token.target_email,
          ])
        ).rowCount
      )
        throw failure(400, "Email change unavailable.");
      await deliverDevOrLogEmail(
        {
          userId: user.id,
          to: user.email,
          subject: "Chix primary email changed",
          body: "Your primary email was changed. Review account security if you did not request this.",
        },
        c,
      );
      await c.query(
  ...
  app.post("/v1/auth/account-discovery/request", async (req) => {
    const b = parse(
      z.strictObject({
        recoveryEmail: Email,
        username: z.string().trim().max(20).toLowerCase().optional(),
    ...
    await transaction(async (c) => {
      const users = (
        await c.query(
          "SELECT id FROM users WHERE recovery_email=$1 AND recovery_verified_at IS NOT NULL AND suspended_at IS NULL AND ($2::text IS NULL OR username=$2) ORDER BY id LIMIT 5 FOR UPDATE",
          [b.recoveryEmail, b.username ?? null],
        )
      ).rows;
  ...
  app.post("/v1/auth/account-discovery/confirm", async (req) => {
    ...
    return transaction(async (c) => {
      ...
      if (
        user.recovery_email !== token.target_email ||
        !user.recovery_verified_at
      )
        throw failure(400, "Invalid or expired link.");
