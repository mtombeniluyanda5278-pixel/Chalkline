...
const profile = z.strictObject({
  ...
  curriculum: z.enum(["CAPS", "IEB", "Cambridge", "custom"]),
  customCurriculum: z.string().trim().max(100).optional(),
  phases: z
    .array(z.enum(["Foundation", "Intermediate", "Senior", "FET", "Other"]))
    .max(5),
  grades: list,
  subjects: list,
});
...
export function isBirthday(dob: string, zone: string, now = new Date()) {
  return dateInZone(zone, now).slice(5) === dob.slice(5);
}
export async function registerPreferenceRoutes(app: FastifyInstance) {
  app.get("/v1/config", async () => ({
    appName: config.APP_NAME,
    bot: {
      provider: config.BOT_PROTECTION_PROVIDER,
      siteKey: config.BOT_PROTECTION_SITE_KEY,
      registrationRequired: config.BOT_REQUIRE_REGISTRATION,
    },
  }));
  app.get("/v1/me/preferences", async (req, reply) => {
    ...
    const flags = (
      await pool.query(
        "SELECT f.name,f.enabled,f.percentage,o.enabled AS override FROM feature_flags f LEFT JOIN feature_flag_overrides o ON o.flag=f.name AND o.user_id=$1",
        [user.id],
      )
    ).rows;
    ...
    return {
      ...
      role: row.role,
      features,
      today: dateInZone(row.timezone),
      birthday:
        features.birthday &&
        row.personal_touches &&
        isBirthday(row.date_of_birth, row.timezone),
  ...
  app.patch("/v1/me/preferences", async (req, reply) => {
    const user = await requireUser(req, reply);
    if (!user) return;
    await limit("accountChange", user.id);
    const b = parse(
      z.strictObject({
        sessionDays: z
  ...
  app.get("/v1/trash", async (req, reply) => {
    ...
    return {
      items: (
        await pool.query(
          "SELECT id,title,kind,trashed_at FROM documents WHERE user_id=$1 AND trashed_at IS NOT NULL UNION ALL SELECT id,title,'file',trashed_at FROM resources WHERE user_id=$1 AND status='trashed' ORDER BY trashed_at DESC LIMIT 50",
          [user.id],
        )
      ).rows,
    };
  ...
  for (const kind of ["documents", "resources"] as const) {
    app.post(`/v1/trash/${kind}/:id/restore`, async (req, reply) => {
      ...
      const sql =
        kind === "documents"
          ? "UPDATE documents SET trashed_at=NULL WHERE id=$1 AND user_id=$2 AND trashed_at IS NOT NULL AND trashed_at>now()-($3::int*interval '1 day')"
          : "UPDATE resources SET status=CASE WHEN previous_status='ready' AND scanned_at IS NOT NULL THEN 'ready' ELSE 'scan_failed' END,trashed_at=NULL WHERE id=$1 AND user_id=$2 AND status='trashed' AND previous_status<>'quarantined' AND trashed_at>now()-($3::int*interval '1 day')";
      if (
        !(await pool.query(sql, [id, user.id, config.TRASH_RETENTION_DAYS]))
          .rowCount
      )
        throw failure(404, "Item unavailable.");
      return { ok: true };
    });
    app.delete(`/v1/trash/${kind}/:id`, async (req, reply) => {
      ...
      if (kind === "documents") {
        if (
          !(
            await pool.query(
              "DELETE FROM documents WHERE id=$1 AND user_id=$2 AND trashed_at IS NOT NULL",
              [id, user.id],
            )
          ).rowCount
        )
        ...
        if (
          !(
            await pool.query(
              "SELECT id FROM resources WHERE id=$1 AND user_id=$2 AND status='trashed'",
              [id, user.id],
            )
          ).rowCount ||
          !(await queuePurge(id, user.id))
        )
          throw failure(404, "Item unavailable.");
      }
      return { ok: true };
